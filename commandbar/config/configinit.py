...
from commandbar.config import (config, configdata, configfiles, configtypes,
                                configexc, configcommands, stylesheet, qtargs)
from commandbar.utils import usertypes, log, standarddir, message, objreg
from commandbar.config import configcache
from commandbar.misc import objects, savemanager #, msgbox
...
def late_init(save_manager: savemanager.SaveManager) -> None:
    ...
    if _init_errors is not None:
        ...
        #                        plain_text=False)
        # errbox.exec()
        log.config.error(f"Error while reading config {_init_errors}")
