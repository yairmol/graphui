...
# import qutebrowser
from commandbar.utils import log, utils
...
def is_git_repo() -> bool:
    ...
    # TODO: change (qutebrowser.basedir)
    gitfolder = os.path.join(base_dir, os.path.pardir, '.git')
...
def docs_up_to_date(path: str) -> bool:
    ...
    if hasattr(sys, 'frozen') or not is_git_repo():
        return True
    html_path = os.path.join(base_dir, 'html', 'doc', path)
    filename = os.path.splitext(path)[0]
    asciidoc_path = os.path.join(base_dir, os.path.pardir,
